...
- **Replicate other studies in using GNNs or Transformers on the full dataset** — I want to try replicating what other studies have done with classifiers, CNN-RNNS, GNNs and Transformers on the full dataset (all three tasks). If I reproduce the study's architecture and evaluation protocol and still cannot approach the reported result, that gives me a reason to investigate differences in my upstream pipeline. I actually understand why the later two methods could do better here. Running a CNN-RNN on all three tasks means learning spatial filters that identify task-relevant regional activity and then using the temporal patterns of that activity to distinguish between the three directional commands. A GNN could potentially capture additional information by learning how activity across different regions is related during each task, which in this case could be more beneficial because we have to train the model on multiple tasks which activate different parts of the brain. A Transformer could do something similar through attention, without needing to define a fixed graph up front.
